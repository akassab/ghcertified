...
- <input type="checkbox" disabled> Configure aprobaciones requeridas para ejecuciones desde forks en el repositorio
- <input type="checkbox" disabled> Configure reglas de protección de despliegue para el repositorio  
- <input type="checkbox" disabled> Configure reglas de protección de ramas para el repositorio
- <input type="checkbox" disabled> El flujo de trabajo no se desencadenará desde forks si se utiliza el evento `pull_request`. Si desea hacerlo, debe usar el disparador de evento `fork_pull_request` con el indicador `require-approval`.
