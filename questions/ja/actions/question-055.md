...
- <input type="checkbox" disabled> `commit A` のコードで実行されます
- <input type="checkbox" disabled> `commit B` のコードで実行されます
- <input type="checkbox" disabled> GitHub Actionsではワークフローを再実行することはできません。新しいワークフローをトリガーする必要があり、それは最新の変更で実行されます
- <input type="checkbox" disabled> `commit A` のコードで1回、`commit B` のコードで1回、2つのワークフローがトリガーされます
