...
- <input type="checkbox" disabled> シークレットまたは変数に `ACTIONS_RUNNER_DEBUG` という名前を設定し、値を `true` にする  
- <input type="checkbox" disabled> ワークフローを「Enable debug logging（デバッグログの有効化）」を選択して再実行する  
- <input type="checkbox" disabled> ワークフローのリポジトリに `ACTIONS_RUNNER_DEBUG` というトップレベルフォルダーを追加する  
- <input type="checkbox" disabled> 使用されているセルフホスト型ランナーの `_diag` ディレクトリに `runner-diagnostic-logs` サブフォルダーを追加する  
- <input type="checkbox" disabled> セルフホスト型ランナーの `_diag` ディレクトリを `runner-diagnostic-logs` にリネームする  
